...
static getSurveysBySurveyorAndDay(surveyor_code, dayMilli){
  return appointments.filter((survey)=>{
    return(survey.moveware_employee_code == surveyor_code && survey.dateMilli == dayMilli) 
  })
}
...
static getSurveysByBranchDayAndSurveyor(dayMilli, branch_code, surveyor_code){
  var surveys = appointments.filter((survey)=>{
    return(survey.dateMilli == dayMilli && survey.branch_code == branch_code && survey.moveware_employee_code == surveyor_code)
  })
...
static getAllSurveyorsByBranch(branch_code){
  ...
  appointments.forEach((survey)=>{
    if(survey.branch_code==branch_code && !branch_surveyors.includes(survey.moveware_employee_code)){
