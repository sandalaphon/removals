  ...
  var surveyors_hidden = store.getState().surveyor.surveyors_hidden
    ...
    surveyors.forEach((surveyor_code)=>{
      ...
      var driveWaitObject = {} 
       ...
       var routeDisplayObj = mapObjectInstances['surveyor'].surveyRoutesByCode[dateMilli+surveyor_code]
       if(!routeDisplayObj) return

        legs = routeDisplayObj.directions.routes[0].legs
       var travelTimes = setTravelTimeOfEachLegArray(legs)
       var surveyors_surveys = Appointment. getSurveysByBranchDayAndSurveyor(dateMilli, selected_branch, surveyor_code)
      ...
      legs.forEach((leg, i)=>{
        ...
        if(i==0){
          ...
          departureTimeMilli = nextAppointmentTime - travelTimes[i] - tenMinsContingencyInMilli
        ...
        if(surveyNext){
          ...
          surveyFinishTime = surveyNext.milliseconds_since_1970 + timeSpentAtEachSurvey30minsMilli
          driveWaitObject[surveyFinishTime] = 'drive'
          departureTimeMilli = surveyFinishTime 
        } 
      }) 
      var sliderMarkerObj = getSliderMarkerObject(sliderSecondsFromStart, dateMilli, driveWaitObject, legs, surveyor_code)
      sliderMarkerObjects.push(sliderMarkerObj)
    })

    mapObjectInstances['surveyor'].handleSliderMarkerArray(sliderMarkerObjects, false)
 
}
 

 function getSliderMarkerObject(sliderSecondsFromStart, dateMilli, driveWaitObject, legs, surveyor_code){
  ...
  Object.keys(driveWaitObject).forEach((milli, index)=>{
   ...
   if( +milli < timeMilli ) {
    ...
    if(endOfLeg < timeMilli && Math.floor(indexX/3)==legs.length-1) indexOfLeg = NaN
  }
  ...
  var indexOfLeg = Math.floor(indexX/3)
   ...
   var sliderMarkerObject = getSurveyorSliderMarkerObject(legs[indexOfLeg].steps, secondsFromStartOfStep, iconCode, surveyor_colour, surveyor_code)
...
function getSurveyorSliderMarkerObject(steps, secondsFromStart, iconCode, surveyor_colour, surveyor_code){ 
  var secondsToStep = 0
  var sliderMarkerObject = {}
  var stepCompleted = false

if(iconCode != 'drive'){
  var markerCoords={lat: steps[steps.length-1].end_point.lat(), lng: steps[steps.length-1].end_point.lng()}
  return {markerCoords, colour: surveyor_colour, message: surveyor_code,leg: iconCode}
}
  steps.forEach((step)=>{
    ...
    if(secondsToStep>secondsFromStart){
      var fractionOfStep  = (  secondsFromStart  -  (secondsToStep-currentStepDuration))/currentStepDuration
      ...
      sliderMarkerObject = {markerCoords, colour: surveyor_colour, message: surveyor_code,leg: iconCode}
      stepCompleted = true
    }
  });
