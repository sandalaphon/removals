...
static getTripsByDateRangeAndBranch(dateRange, branch_code){
  ...
  var dateMilliArray = []
  if (difference == 0) return Trip.getTripsByDayMilliAndBranch(dateRange.start_date, branch_code)
    for(var i = dateRange.start_date; i<dateRange.end_date; i = i + 86400000){
  ...
  if(branch_code=='All_Branches'){
    ...
    return trips.filter((trip)=>{
      return (trip.branch_code===branch_code && dateMilliArray.includes(trip.dateMilli))
    })
  }
}
