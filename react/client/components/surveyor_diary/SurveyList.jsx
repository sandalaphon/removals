...
show_overview_table(){
  if(!this.props.all_surveys || !this.props.all_branches || !this.state.mapObject) return <div></div>
  ...
  this.branch_surveyors = Appointment.getAllSurveyorsByBranch(this.selected_branch)
  var headerStyle = {backgroundColor: 'gainsboro'}
  var tableStyle = { tableLayout: 'fixed', width: '50vw' }
  var tdStyle20 = { width: '20%' }
  var tdStyleSurveyorHeader = { width: `${80/this.branch_surveyors.length}%` }
  var surveyors_headers = this.branch_surveyors.map((surveyor_code)=>{

    return (<td id={surveyor_code} onClick = {this.handleSurveyorOverviewClick.bind(this)} style = {tdStyleSurveyorHeader}><b>{surveyor_code}</b></td>)
  })
  return (
...
get_overview_table_body(){
  ...
  var surveyors_surveys = Appointment.getSurveysByBranchDayAndSurveyor(currentDayMilli, this.selected_branch, surveyor_code)
...
//// draw surveyor routes
  ...
  for(var i = 0; i<19; i++){
    ...
    this.branch_surveyors.forEach((surveyor_code)=>{
      ...
      survey_now ? tds.push(<td>{survey_now.survey_id}</td>) : tds.push(<td>No Appointment</td>)

    })
    table_rows.push(
      <tr><td>{time_to_display}</td>
       {tds}
      </tr>
     )
  }
