...
class EnterCosts extends React.Component{
  ...
  componentDidMount(){
    this.props.getCostsFromRails()
    var ids = [ "18t_fuel_per_mile", "9t_fuel_per_mile", "3.5t_fuel_per_mile", "18t_driver_per_hour", "9t_driver_per_hour", "9t_driver_per_hour",
  "luton_driver_per_hour",  "porter_per_hour" ]
   ids.forEach((id)=>{
...
getList(){
  ...
  var costs_titles = ['18t fuel per mile', '9t fuel per mile', '3.5t fuel per mile', '18t driver per hour', '9t driver per hour', 'Luton driver per hour', 'Porter per hour' ]
  ...
  for(var i = 0; i < costs_titles.length; i++){
    table_rows.push(
      ...
      <td>{cost_inputs[i]}</td>
      <td><Button bsStyle='info' id = {Object.keys(costs)[i]} onClick = {this.handleUpdateClick.bind(this)}>Update</Button></td>
      </tr>
      )
  }
