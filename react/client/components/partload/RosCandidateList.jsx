...
if(diversion.single_trip_solution){
  move_explanation = 
  `Moveware Job Number ${diversion.undiverted_job.moveware_code} departs ${diversion.undiverted_job.branch_code} ${undiverted_trip_departure_date}. 
  After delivering goods to ${diversion.undiverted_job.client_name} at ${diversion.undiverted_job.delivery_postcode} this truck now continues to ${diversion.out_of_store_job.branch_code} Branch where ${diversion.out_of_store_job.volume} cuft of ${diversion.out_of_store_job.client_name}'s stored goods are collected in crates.
   The truck now returns home to ${diversion.undiverted_job.branch_code}. 
   On ${delivery_date} the goods are delivered to the client at ${diversion.out_of_store_job.delivery_postcode}`
}else{
  move_explanation = 
  `Moveware Job Number ${diversion.undiverted_job.moveware_code} departs ${diversion.undiverted_job.branch_code} ${undiverted_trip_departure_date}. 
  After delivering goods to ${diversion.undiverted_job.client_name} at ${diversion.undiverted_job.delivery_postcode} this truck now diverts to ${diversion.out_of_store_job.branch_code} Branch where ${diversion.out_of_store_job.volume} cuft of ${diversion.out_of_store_job.client_name}'s stored goods are collected in crates. 
  The truck now delivers these crates to ${diversion.optimal_branch.branch_code} Branch where the goods are now stored awaiting delivery. 
  On ${delivery_date} ${diversion.optimal_branch.branch_code} delivers the goods to the client at ${diversion.out_of_store_job.delivery_postcode}.`
}
          ...
          if(this.props.ros_requested_ids.includes(row.original.id)){
            ...
            subComponent1 =  (<ReactTable
                         ...
                         SubComponent    = { row => {
                                              ...
                                              return(
                                                      <div><p>{explanation_string}</p></div>
                                                    )
