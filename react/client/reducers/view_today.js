...
function handleTripData(state = {
  ...
  today_date_range: {start_date: todayStartMidnight(), end_date: todayStartMidnight()},
  today_trips: getTodayTrips({start_date: todayStartMidnight(), end_date: todayStartMidnight()}, 'All_Branches'),
},action){
  ...
  switch(action.type) {
    ...
    var trips = getTodayTrips(state.today_date_range, state.today_branch_selected)
